window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region dsh-skills-manage styles
		const CSS = `
...
.skm-name-sub{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;color:var(--dsw-alias-label-secondary,#888);}
...
`;
		const CSS_ID = "dsh-skills-manage";
		if (typeof document !== "undefined" && document.querySelector(`style[data-plugin-css="${CSS_ID}"]`) === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-skills-manage";
		...
		function SkillForm(props) {
			...
			return h("div", { className: "skm-form" },
				...
				Field({ label: "调用名 name（小写 kebab-case，如 daily-work-log；显示名默认取目录名）", value: name, onChange: (e) => setName(e.target.value) }),
		...
		function SkillManager(props) {
			...
			const visible = items.filter((s) => {
				...
				if (!q) return true;
				return (s.displayName + " " + s.name + " " + (s.description || "")).toLowerCase().includes(q);
			});

			const openDetail = (name) => {
				setEditing(false);
				// Toggle: clicking the already-open card collapses it again.
				if (selected === name) {
					setSelected(null);
					setDetail(null);
					setDetailError(null);
			...
			const confirmDelete = (e) => {
				e.stopPropagation();
				if (!detail) return;
				if (!window.confirm(`确定删除技能「${detail.displayName || detail.name}」？\n将删除整个技能目录（含附属文件），此操作不可恢复。`)) return;
				setDeleting(true);
			...
			const deleteFromMenu = () => {
				...
				if (!window.confirm(`确定删除技能「${item.displayName || item.name}」？\n将删除整个技能目录（含附属文件），此操作不可恢复。`)) return;
				setDeleting(true);
				...
				body = h("div", { className: "skm-list" },
					...
					visible.map((s) => {
						const head = h("div", { className: "skm-card-head" },
							h("span", { className: "skm-name" }, s.displayName || s.name),
							s.displayName && s.displayName !== s.name && h("span", { className: "skm-name-sub" }, s.name),
						);
						...
						if (selected === s.name) {
							...
							if (detailError) {
								...
								const rows = [];
								if (detail.displayName && detail.displayName !== detail.name) {
									rows.push(h("div", { className: "skm-path", key: "dn" }, "显示名（目录名）：" + detail.displayName));
									rows.push(h("div", { className: "skm-path", key: "inv" }, "调用名（name）：" + detail.name));
								}
						...
						return h("div", {
							...
							onClick: () => { closeMenu(); openDetail(s.name); },
