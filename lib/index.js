...
export function apply(ctx) {
  ...
  register(`${API_PREFIX}/update`, async (req, res) => {
    try {
      ...
      const newDir = join(userSkillRoot, skillName)
      if (oldDir !== newDir) {
        // Rename the whole skill directory (keeps sidecar files like state.json).
        try {
          await readFile(join(newDir, 'SKILL.md'), 'utf8')
          return json(res, 409, { error: `技能 "${skillName}" 已存在` })
        } catch (e) {
          if (e.code !== 'ENOENT') throw e
        }
        await rename(oldDir, newDir)
        await writeFile(join(newDir, 'SKILL.md'), buildSkillFile(data, content), 'utf8')
        json(res, 200, { ok: true, name: skillName, path: join(newDir, 'SKILL.md') })
      } else {
